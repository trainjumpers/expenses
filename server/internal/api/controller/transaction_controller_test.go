...
var _ = Describe("TransactionController", func() {
	...
	Describe("CreateTransaction", func() {
		It("should create a transaction for user 2", func() {
			...
			resp, response := testHelperUser1.MakeRequest(http.MethodPost, "/account", accInput)
			...
			resp, response = testHelperUser1.MakeRequest(http.MethodPost, "/transaction", txInput)
			Expect(resp.StatusCode).To(Equal(http.StatusCreated))
		...
		It("should not allow user 3 to access user 1's transaction", func() {
			url := "/transaction/1"
			resp, _ := testHelperUser2.MakeRequest(http.MethodGet, url, nil)
		...
		It("should create a transaction with category mappings", func() {
			...
			resp, response := testHelperUser1.MakeRequest(http.MethodPost, "/transaction", input)
			Expect(resp.StatusCode).To(Equal(http.StatusCreated))
		...
		It("should return error for invalid JSON", func() {
			resp, _ := testHelperUser1.MakeRequest(http.MethodPost, "/transaction", "{ name: invalid json }")
			Expect(resp.StatusCode).To(Equal(http.StatusBadRequest))
		})

		It("should return error for empty body", func() {
			resp, _ := testHelperUser1.MakeRequest(http.MethodPost, "/transaction", "")
			Expect(resp.StatusCode).To(Equal(http.StatusBadRequest))
		})
		...
		It("should handle string amount gracefully", func() {
			...
			resp, _ := testHelperUser1.MakeRequest(http.MethodPost, "/transaction", requestBody)
			Expect(resp.StatusCode).To(Equal(http.StatusBadRequest))
		})
		...
		It("should return error for duplicate transaction", func() {
			...
			resp, _ = testHelperUser1.MakeRequest(http.MethodPost, "/transaction", input)
		...
		It("should not allow adding category mapping of a different user on create", func() {
			...
			catResp, catResponse := testHelperUser1.MakeRequest(http.MethodPost, "/category", catInput)
	...
	Describe("ListTransactions", func() {
		...
		It("should handle invalid filter values gracefully", func() {
			// Invalid account_id
			resp, _ := testHelperUser1.MakeRequest(http.MethodGet, "/transaction?account_id=invalid", nil)
			Expect(resp.StatusCode).To(Equal(http.StatusOK)) // Should ignore invalid filter

			// Invalid category_id
			resp, _ = testHelperUser1.MakeRequest(http.MethodGet, "/transaction?category_id=invalid", nil)
			Expect(resp.StatusCode).To(Equal(http.StatusOK)) // Should ignore invalid filter

			// Invalid amount
			resp, _ = testHelperUser1.MakeRequest(http.MethodGet, "/transaction?min_amount=invalid", nil)
			Expect(resp.StatusCode).To(Equal(http.StatusOK)) // Should ignore invalid filter

			// Invalid date
			resp, _ = testHelperUser1.MakeRequest(http.MethodGet, "/transaction?date_from=invalid", nil)
			Expect(resp.StatusCode).To(Equal(http.StatusOK)) // Should ignore invalid filter
		})
	...
	Describe("UpdateTransaction", func() {
		...
		It("should return error when trying to update transaction of different user", func() {
			...
			resp, response := testHelperUser1.MakeRequest(http.MethodPatch, url, input)
			Expect(resp.StatusCode).To(Equal(http.StatusNotFound))
			Expect(response["message"]).To(Equal("transaction not found"))
		})
	...
	Describe("DeleteTransaction", func() {
		It("should delete transaction by id", func() {
			...
			resp, response := testHelperUser1.MakeRequest(http.MethodPost, "/transaction", createTransactionInput)
			Expect(resp.StatusCode).To(Equal(http.StatusCreated))
			...
			resp, _ = testHelperUser1.MakeRequest(http.MethodGet, "/transaction/"+strconv.FormatInt(createdId, 10), nil)
			Expect(resp.StatusCode).To(Equal(http.StatusNotFound))
		})
