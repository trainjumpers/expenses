...
CREATE TABLE ${DB_SCHEMA}.rule_action (
   ...
   created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
   updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
   CONSTRAINT fk_rule FOREIGN KEY (rule_id) REFERENCES ${DB_SCHEMA}.rules(id)
);
...
BEFORE UPDATE ON ${DB_SCHEMA}.rules
FOR EACH ROW
...
DROP TRIGGER IF EXISTS update_rules_modtime ON ${DB_SCHEMA}.rules;
DROP TABLE IF EXISTS ${DB_SCHEMA}.rule_condition;
DROP TABLE IF EXISTS ${DB_SCHEMA}.rule_action;
DROP TABLE IF EXISTS ${DB_SCHEMA}.rules;
-- +goose StatementEnd 