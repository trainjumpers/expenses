...
CREATE TABLE IF NOT EXISTS ${DB_SCHEMA}.transaction (
    ...
    CONSTRAINT fk_transaction_account_id FOREIGN KEY (account_id) REFERENCES ${DB_SCHEMA}.account(id)
);

-- Create index for better query performance
CREATE INDEX idx_transaction_account_id ON ${DB_SCHEMA}.transaction(account_id);
...
DROP INDEX IF EXISTS ${DB_SCHEMA}.idx_transaction_account_id;
