...
import { Upload, FileText, AlertCircle, ChevronDownIcon, X, Settings } from "lucide-react";
...
export function ImportStatementModal({
  ...
  const [isCustomParserOpen, setIsCustomParserOpen] = useState(false);
  ...
  const handleCustomParser = () => {
    onOpenChange(false); // Close this modal
    setIsCustomParserOpen(true); // Open custom parser modal
  };
  ...
  return (
    <>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-6 py-4">
              ...
              <div className="bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  <Settings className="h-5 w-5 text-amber-500 dark:text-amber-400 mt-0.5" />
                  <div className="flex-1">
                    <div className="text-sm text-amber-700 dark:text-amber-300">
                      <p className="font-medium mb-1">Need Custom Column Mapping?</p>
                      <p className="text-xs text-amber-600 dark:text-amber-400 mb-2">
                        If your bank statement has a different format, use our custom parser to map columns manually.
                      </p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={handleCustomParser}
                      className="text-amber-700 border-amber-300 hover:bg-amber-100 dark:text-amber-300 dark:border-amber-700 dark:hover:bg-amber-950/50"
                    >
      ...
      <CustomParserModal 
        isOpen={isCustomParserOpen} 
        onOpenChange={setIsCustomParserOpen} 
      />
    </>
  );
}
